...
//        when(mockMapLoader.validateMap()).thenReturn(true);
	...
	public void testSetterGetterMethods() {
		...
		assertEquals("getCurrentPhase should return the set phase", GameController.STARTUP_PHASE,
	...
	public void testMapEditorControllerHandleCommand() {
		...
		boolean result = mapEditorController
				.handleCommand(new String[] { "editcontinent", "-add", "NewContinent", "3" }, "editcontinent", true);
		assertTrue("handleCommand should return true", result);

		// Test adding a country
		result = mapEditorController.handleCommand(new String[] { "editcountry", "-add", "NewCountry", "NewContinent" },
				"editcountry", true);
		assertTrue("handleCommand should return true", result);
	...
	public void testHandleReinforcement() {
		...
		assertEquals("Player with 6 territories should get 3 reinforcements", 3,
				d_players.get(0).getNbrOfReinforcementArmies());
		assertEquals("Player with 3 territories should get minimum 3 reinforcements", 3,
				d_players.get(1).getNbrOfReinforcementArmies());
	}
}