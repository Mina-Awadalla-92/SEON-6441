 ...
 * 
 * These tests cover:
 * 1. Successful bombing where the target territory's armies are halved.
 * 2. Cancellation of the bomb order due to active diplomacy between the issuer and the target's owner.
 */
public class BombOrderTest {
    ...
    /**
     * Test successful execution of BombOrder:
     * The territory's army count should be halved.
     */
    @Test
    public void testBombOrderExecution_Success() {
        ...
        // After execution, the target territory's armies should be halved (20 / 2 = 10).
        assertEquals("TargetLand should have half its original armies (20/2 = 10) after bomb execution.",
                     10, targetTerritory.getNumOfArmies());
    }

    /**
     * Test that the BombOrder is cancelled when there is active diplomacy:
     * If the target territory's owner is in the issuer's negotiated players list,
     * the bomb order should be undone and the army count remains unchanged.
     */
    @Test
    public void testBombOrderExecution_CancelledDueToDiplomacy() {
        // Add the target territory's owner (defender) to the issuer's negotiated players list.
        ...
        // Since diplomacy is active, the bomb order should be cancelled and the army count should remain unchanged.
