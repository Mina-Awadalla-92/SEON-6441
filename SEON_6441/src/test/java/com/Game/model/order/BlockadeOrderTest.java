 ...
 *
 * Tests include: 1. Order validation failure when no Blockade card is
 * available. 2. Successful execution: the territory becomes neutral and its
 * armies are tripled.
 */
public class BlockadeOrderTest {
	...
	/**
	 * Test that the blockade order fails validation if the player does not have a
	 * Blockade card.
	 */
	@Test
	public void testBlockadeOrderValidationFailsWithoutCard() {
		...
		// Since validateBlockade calls removeCard(CardType.BLOCKADE) and there is no
		// such card,
	...
	/**
	 * Test that a valid blockade order correctly converts the target territory to
	 * neutral and triples its armies.
	 */
	@Test
	public void testBlockadeOrderExecution() {
		// First, add a Blockade card to the player's card collection so the order can
		// be validated.
