...
/**
 * Demonstrates tests for Airlift orders (move & attack) covering:
 * 1) Order validation,
 * 2) Conquering a territory,
 * 3) Moving armies post-conquest,
 * 4) End-of-game scenario when all territories are conquered.
 */
public class AirliftOrderTest {
    ...
    /**
     * 1) Test order validation: 
     * Attempt to airlift from a territory the player does NOT own -> should fail validation.
     */
    @Test
    public void testAirliftOrderValidation_FailWrongOwner() {
        ...
        boolean result = attacker.issueOrder(invalidCommand, testMap, 
                                             Arrays.asList(attacker, defender, thirdPlayer));
        assertFalse("Order validation should fail when source territory isn't owned by the issuer", result);
    }

    /**
     * 2) Test conquering a country:
     * Attacker uses AirliftAttack on Defender's territory with enough armies to ensure conquest.
     * We expect the territory to be owned by the Attacker afterwards.
     */
    @Test
    public void testAirliftAttack_ConquerEnemyTerritory() {
        ...
        boolean result = attacker.issueOrder(conquerCommand, testMap, 
                                             Arrays.asList(attacker, defender, thirdPlayer));
        ...
        assertEquals("EnemyLand should be owned by Attacker after a successful conquest",
                     attacker, enemyTerritory.getOwner());
        // The territory's army count should reflect the surviving attacking armies (>= 0).
        assertTrue("EnemyLand should have some attacking armies placed after conquest",
                   enemyTerritory.getNumOfArmies() > 0);
    }

    /**
     * 3) Test moving armies in a conquered country:
     * - Use AirliftMove to a neutral territory -> it becomes conquered.
     * - Then check that the newly conquered territory has the moved armies.
     */
    @Test
    public void testAirliftMove_ConquerNeutralAndMoveArmies() {
        ...
        boolean result = attacker.issueOrder(moveCommand, testMap, 
                                             Arrays.asList(attacker, defender, thirdPlayer));
        ...
        assertEquals("NeutralLand should be owned by Attacker after being airlifted to", 
                     attacker, neutralTerritory.getOwner());
        assertEquals("NeutralLand should have 4 armies after the move", 
                     4, neutralTerritory.getNumOfArmies());
    }

    /**
     * 4) Test end-of-game scenario:
     * - If the attacker conquers the last territory not owned by them, the game ends.
     * - We'll simulate that the attacker has already conquered "EnemyLand" and "NeutralLand"
     *   so only "ThirdLand" remains. Then we do one more conquest to end the game.
     */
    @Test
    public void testAirliftAttack_EndOfGame() {
        // Assume attacker already owns "EnemyLand" and "NeutralLand" (simulate a mid-game scenario)
    	attacker.addCard(CardType.AIRLIFT);
        enemyTerritory.setOwner(attacker);
        neutralTerritory.setOwner(attacker);

        // So the only territory not owned by Attacker is "ThirdLand" (owned by thirdPlayer).
        ...
        boolean result = attacker.issueOrder(finalConquest, testMap, 
                                             Arrays.asList(attacker, defender, thirdPlayer));
        ...
        boolean allConquered = allTerritories.stream()
                                             .allMatch(t -> t.getOwner() == attacker);
